...
def filter(files,extensions):
    result =[]
    for filename in files:#öğeleri kontrol et
        for ext in extensions:#öğeleri kontrol et
            if filename.endswith(ext):#dosya isminin son 3 öğesini kontrol et
                result.append(filename)
    return result#tüm heryerden ulaşalım diye


#2.hafta
def chooseWorkdir():
    global workdir
    workdir =QFileDialog.getExistingDirectory()#Dizin haline getir.Liste


def showFilenamesList():
    extensions=['.jpg','.jpeg','.png','.gif','.bmp']#uzantıları ekle
    chooseWorkdir()
    filenames=filter(os.listdir(workdir),extensions)#her şeyi filenames e kaydet.
    lw_files.clear()
...
class ImagePrecessor():
    def __init__(self):
        ...
        self.dir=None #Geçerli dosya yolu
        self.filename=None#Geçerli dosya adı
        self.save_dir="Modified/"#Değiştirilen dosyanının kaydedileceği yer.
...
workimage=ImagePrecessor()  #iş için mevcut çalışma resmi
lw_files.currentRowChanged.connect(showChosenImage)#dosyalardan bir öğe seçilince showchosen cağır.
